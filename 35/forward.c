...
void loop() {
  ...
  digitalWrite(forward_IN3, LOW); 
  digitalWrite(forward_IN4, HIGH); 
  analogWrite(forward_ENABLE_B, 255);

  // --- 前右輪 (forward_ENABLE_A) ---
  digitalWrite(forward_IN1, LOW); 
  digitalWrite(forward_IN2, HIGH); 
  analogWrite(forward_ENABLE_A, 255);

  // --- 後左輪 (back_ENABLE_A) ---
  digitalWrite(back_IN1, HIGH); 
  digitalWrite(back_IN2, LOW); 
  analogWrite(back_ENABLE_A, 255);

  // --- 後右輪 (back_ENABLE_B) ---
  digitalWrite(back_IN3, HIGH); 
  digitalWrite(back_IN4, LOW); 
  analogWrite(back_ENABLE_B, 255);
